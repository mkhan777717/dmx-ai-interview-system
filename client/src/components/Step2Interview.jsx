...
function Step2Interview({ interviewData, onFinish }) {
  ...
  useEffect(() => {
    ...
    recognition.continuous = false;  // use false + auto-restart for reliability
    ...
    recognition.onresult = (event) => {
      ...
      // Only append when the result is final (user paused)
      if (result.isFinal) {
        setAnswer((prev) => prev + " " + transcript);
      }
    };

    // Auto-restart after silence so mic stays alive
    recognition.onend = () => {
  ...
  const stopMic = () => {
    if (recognitionRef.current) {
      isMicOnRef.current = false; // prevent auto-restart in onend
      try {
