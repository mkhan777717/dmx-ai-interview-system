...
function Navbar() {
  ...
  return (
    <div className='bg-transparent flex justify-center px-4 pt-6 absolute top-0 w-full z-50'>
        <motion.div 
        initial={{opacity:0 , y:-40}}
        animate={{opacity:1 , y:0}}
        transition={{duration: 0.3}}
        className='w-full max-w-[1200px] bg-white rounded-full shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 px-6 py-3 flex items-center relative'>
            ...
            <div className='flex-1 flex items-center gap-2 cursor-pointer' onClick={() => navigate('/')}>
                ...
                <h1 className='font-bold hidden md:block text-xl tracking-tight'>InterviewIQ<span className="text-green-500">.AI</span></h1>
            </div>

            {/* Center: Nav links */}
            <div className='flex-1 flex justify-center items-center gap-8 text-sm font-semibold text-gray-500'>
              <button onClick={() => navigate('/v2/interview')} className='hover:text-black transition'>Start Interview</button>
              <button onClick={() => navigate('/history')} className='hover:text-black transition'>History</button>
            </div>
